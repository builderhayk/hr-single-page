    ...
    for (let i = 0; i < uniqueIds.length; i++) {
        ...
        sortedUserData.forEach(user => {
            ...
            if (user && user.withTimeInput) {
                user.time = moment(user.dateInputValue).format('YYYY-MM-DD') + ' ' + user.timeInputValue;
            }
            if (user && user.id === uniqueIds[i] && user.attendanceCheckPoint === 'Entrance_Entrance_Entrance Card Reader1') {
                enteredDate = moment(user.time, 'YYYY-MM-DD HH:mm:ss');
            }

            if (user && user.id === uniqueIds[i] && user.attendanceCheckPoint === 'Exit_Exit_Entrance Card Reader1') {
        ...
        if (totalMinutes >= 60) {
            result[i].totalWorkingTime = `hours ${totalHours + Math.floor(totalMinutes/60)} : minutes ${totalMinutes%60}`;
        } else {
            result[i].totalWorkingTime = `hours ${totalHours} : minutes ${totalMinutes}`;
        }
    }
